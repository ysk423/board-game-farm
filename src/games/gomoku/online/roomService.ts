...
export async function submitMove(roomId: string, row: number, col: number, color: StoneColor): Promise<void> {
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (win || draw) {
      tx.set(doc(collection(db, PLAY_RECORDS_COLLECTION)), buildOnlinePlayRecord('gomoku', win ? roleLabel(color) : null));
    }
  });
}
...
export async function resign(roomId: string, color: StoneColor): Promise<void> {
  ...
  batch.set(doc(collection(db, PLAY_RECORDS_COLLECTION)), buildOnlinePlayRecord('gomoku', roleLabel(winner)));
  await batch.commit();
}
