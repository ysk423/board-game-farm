...
export class BoardView {
  ...
  constructor(onCellClick: CellClickHandler) {
    ...
    this.element.className = 'yonmoku-board';
  ...
  render(board: Board, lastMove: Move | null): void {
    for (let row = 0; row < BOARD_SIZE; row++) {
      for (let col = 0; col < BOARD_SIZE; col++) {
        ...
        cell.classList.toggle('yonmoku-cell--black', stone === BLACK);
        cell.classList.toggle('yonmoku-cell--white', stone === WHITE);
        cell.classList.toggle('yonmoku-cell--last', !!lastMove && lastMove.row === row && lastMove.col === col);
  ...
  setInteractive(enabled: boolean): void {
    ...
    this.element.classList.toggle('yonmoku-board--thinking', !enabled);
  }
}
