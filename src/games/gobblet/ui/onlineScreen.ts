...
// オンライン対戦のロビー画面（他ゲームのonlineScreen.tsと同じ構成）
export function renderOnlineScreen(options: OnlineScreenOptions): OnlineScreenView {
  ...
  async function handleCreate(visibilityToUse: Visibility): Promise<void> {
    createError.textContent = '';
    setBusy(true);
    try {
      const roomId = await createRoom(nameInput.value.trim(), visibilityToUse, creatorColor);
