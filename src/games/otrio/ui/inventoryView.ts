...
export class InventoryView {
  ...
  constructor(label: string, onClick: SizeClickHandler | null) {
    ...
    this.element.className = 'otrio-inventory';

    const labelEl = document.createElement('div');
    labelEl.className = 'otrio-inventory__label';
    labelEl.textContent = label;
    this.element.appendChild(labelEl);

    const row = document.createElement('div');
    row.className = 'otrio-inventory__pieces';
    for (const size of SIZES) {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'otrio-inventory__piece';
      if (onClick) {
  ...
  render(inventory: Inventory, selected: Size | null): void {
    for (const [size, button] of this.buttons) {
      ...
      button.classList.toggle('otrio-inventory__piece--selected', selected === size);
    }
  }
}
